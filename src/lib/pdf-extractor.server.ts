...
export async function extractPdf(buffer: Buffer, forceOcr = false): Promise<ExtractedPage[]> {
  const task=getDocument({data:new Uint8Array(buffer),useSystemFonts:true,disableFontFace:true,verbosity:VerbosityLevel.ERRORS})
  let pdf
  try { pdf=await task.promise } catch { throw new Error('The PDF is corrupted, encrypted or password-protected.') }
