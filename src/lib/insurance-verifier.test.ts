...
Fully comprehensive cover. Business use in connection with the motor trade: vehicle recovery business, towing customers' vehicles for hire and reward.`);expect(result.rules.find(r=>r.ruleId==='start-date')?.passed).toBe(true);expect(result.rules.find(r=>r.ruleId==='expiry-date')?.passed).toBe(true)})
 it('recognises a certificate number when a PDF font corrupts certificate',()=>{const result=check(`Cer$ficate of Motor insurance Cer$ficate number: 700H/PPBMT235356
Effective date of commencement 00:00 hrs on 7th August 2026 Date of expiry 12:00 hrs on 7th August 2027
Declared business Vehicle recovery and vehicle collection/delivery/transportation. Use for motor trade business including vehicle recovery and roadside recovery, transportation of customers' vehicles for hire or reward.`);expect(result.rules.find(r=>r.ruleId==='policy-number')?.passed).toBe(true)})
 it('restores dates when PDF table cells put both values outside their labels',()=>{const result=check(`Policy Number: AXARR0034022 Certificate of Motor Insurance
Effective date of the commencement of Insurance for the purpose of relevant law. Date of expiry of Insurance.
23:59 (hours) 09/04/2027 00:00 (hours) 10/04/2026
Business use in connection with the motor trade: vehicle recovery business, towing customers' vehicles for hire and reward.`);expect(result.rules.find(r=>r.ruleId==='started')?.passed).toBe(true);expect(result.rules.find(r=>r.ruleId==='expiry-date')?.passed).toBe(true);expect(result.rules.find(r=>r.ruleId==='active')?.passed).toBe(true)})
...
Use for motor trade purposes. Accident/Breakdown Recovery. Vehicles held in trust or in the custody or control of the policyholder.`);expect(result.rules.find(r=>r.ruleId==='start-date')?.passed).toBe(true);expect(result.rules.find(r=>r.ruleId==='expiry-date')?.passed).toBe(true)})
 it('rejects recovery cover when use for hire or reward is excluded',()=>expect(check(`Certificate of Motor Insurance Tradex Insurance Company PLC
...
Limitations: Use for Motor Trade purposes. Excluding use for hire or reward.`).status).toBe('REJECTED'))
...
Describe your trade or business in full:\nVehicle\nRecovery/Delivery\nYes\n10`;const result=check(text);expect(result.rules.find(r=>r.ruleId==='recovery-cover')?.passed).toBe(true);expect(result.status).toBe('APPROVED')})
 ...
 it('treats a hire-or-reward exclusion as conflicting with commercial recovery',()=>expect(check(`${base}\nUse for hire or reward is excluded.`).rules.find(r=>r.ruleId==='exclusions')?.passed).toBe(false))
 it('rejects a recovery schedule that excludes vehicles used for hire or reward',()=>{const result=check(`${base}\nINSURED VEHICLE: Any motor vehicle held in trust or in the custody or control of the Insured for Motor Trade Purposes EXCLUDING ANY vehicle used for hire or reward or tuition purposes (Use solely for breakdown purposes is not deemed to be used for hire or reward).`);expect(result.status).toBe('REJECTED');expect(result.rules.find(r=>r.ruleId==='exclusions')).toMatchObject({passed:false,page:1});expect(result.rules.find(r=>r.ruleId==='exclusions')?.evidence).toContain('vehicle used for hire or reward')})
 ...
 it('shows the complete AXA does-not-cover section and stops before certification text',()=>{const result=check(`${base}\nThe Policy Does Not Cover Use for the carriage of passengers or goods for hire or reward. Use whilst drawing a greater number of trailers than is permitted by law. Use for motor sports determined by time or speed including practice, racing, pace-making, speed testing, participation in any rally, reliability trial, track days or competitions. Use to secure the release of any motor vehicle which has been seized by or on behalf of any government or public authority which was not the property of the Policyholder or in their custody or control at the time of the seizure. We hereby certify that the policy satisfies the relevant law.`);const evidence=result.rules.find(r=>r.ruleId.startsWith('exclusions-stated-'))?.evidence;expect(evidence).toContain('the policy does not cover use for the carriage');expect(evidence).toContain('release of any motor vehicle');expect(evidence).not.toContain('we hereby certify')})
 it('shows only Tradex exclusion sections and removes the duplicated OCR copy',()=>{const vehicleExclusions='Excluding: Steam driven vehicles. Vehicles owned by Directors, Partners or their Spouses and more specifically insured elsewhere.';const useExclusions='Excluding Carriage of goods for hire and reward other than for motor trade purposes. Use for hire or reward. Use for racing.';const text=`${base} ${vehicleExclusions} Effective time and date of commencement: 09:46 27/01/2026 Operative until: 12:00 21/12/2026 Persons entitled to drive Named driver. Limitations as to use MT: Use for Motor Trade purposes. ${useExclusions} Declaration I hereby certify that the policy satisfies the relevant law. ${vehicleExclusions.replace('Spouses','spouses')} Effective time and date of commencement: 09:46 27/01/2026 ${useExclusions.toLowerCase()} Declaration certificate footer.`;const exclusions=check(text).rules.filter(r=>r.ruleId.startsWith('exclusions-stated-'));expect(exclusions).toHaveLength(2);expect(exclusions[0].evidence).not.toContain('effective time');expect(exclusions[1].evidence).not.toContain('declaration')})
